<script>
	...
	const changeSelectedTopic = () => {
		$page.url.searchParams.set('thema', selectedTopic); 
		history.replaceState(history.state, '', $page.url);
	}
</script>

<div class="collapse collapse-arrow bg-base-200">
	<input type="radio" name="thema" checked={selectedTopic == topic} value={topic} bind:group={selectedTopic} on:change={() => changeSelectedTopic()}/>
