...
<script>
	...
	const fetchChartData = async (event) => {
		...
		history.replaceState(history.state, document.title, `/wortmeldungsverhältnisse/${shownFilterOptions.legislature}/${shownFilterOptions.meetingNumber}/${topicEncoded}?fraktion=${shownFilterOptions.politicalParties}`)
	}
